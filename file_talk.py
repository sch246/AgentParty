...
from tool_executor import execute_blocks, has_do_block, parse_do_blocks
...
class FileHandler:
    ...
    def _process_once(self, source):
        ...
        # 2. 立即消耗触发状态
        if source == "user":
            self._append(f"## {self._display_name()}\n    ")
        ...
        enable_log = self.config.get("log", False)
        log_path = os.path.splitext(self.path)[0] + ".log"

        # 4. LLM 流式写入（带重试）
        self._debug(f"调用 LLM: {model}")
        raw = self._call_llm_with_retry(client, model, messages, log_path, enable_log)
    ...
    def _display_name(self):
        return self.config.get("name") or self.config.get("model", "assistant")
    ...
    def _call_llm_with_retry(self, client, model, messages, log_path, enable_log):
...
def handler(md_path):
    ...
    if md_path not in handlers:
        with open(md_path, encoding="utf-8") as f:
            text = f.read()
        meta, _ = parse_file(text)
        if meta is None:
            return
        config = resolve_config(meta)
        handlers[md_path] = FileHandler(md_path, config)
